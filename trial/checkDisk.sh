...
#command_to_run="echo \"\$( sudo df -h /mnt/safetrax | grep G | awk '{print \$5}' | sed 's/%//'), \$( echo 'root' ; sudo df -h / | grep G | awk '{print \$5}' | sed 's/%//')\" | sed 's/,$//' | sed '$!N;s/.\n/ /'"

command_to_run="echo \"\$(echo ' ,') \$( sudo df -h /mnt/safetrax | grep -E 'T|G' | awk '{print \$5}' | sed 's/%//'), \$( sudo df -h / | grep -E 'T|G' | awk '{print \$5}' | sed 's/%//')\" | sed 's/,$//' | sed '$!N;s/.\n/ /'"

output_file=/home/sukomal/check_services/disk_usage.csv
#date > $output_file
echo "$(date), safetrax, root" > $output_file
