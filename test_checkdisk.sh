...
OUTPUT_FILE="/home/sukomal/check_services/test_checkdisk.csv"
REPO_DIR="$(dirname "$0")"
MAIL_TO="sukomal.das@mtap.in rohan.paunikar@mtap.in"
...
while IFS=',' read -r server mnt root; do
    ...
    if [[ "$mnt" =~ ^[0-9]+$ ]] && (( mnt >= 50 )); then
        ALERT_BODY+="$server has /mnt/safetrax at ${mnt}%\n"
    fi

    if [[ "$root" =~ ^[0-9]+$ ]] && (( root >= 50 )); then
...
if [[ -n "$ALERT_BODY" ]]; then
    ...
    #echo -e "$ALERT_BODY" | mailx -v -s "Disk Usage Alert - $(date '+%Y-%m-%d %H:%M:%S')" \
   # -c "sukomal.das@mtap.in" -b "sukomal.das@mtap.in" "$MAIL_TO"
    #echo -e "$ALERT_BODY" | mailx -v -s "Disk Usage Alert - $(date '+%Y-%m-%d %H:%M:%S')" \
   # -c "sukomal.das@mtap.in" "$MAIL_TO"
fi
